...
**2.** Learn and identify the basic dynamics on building figures following the *grammar of graphics* method; 
...
It provides users with intuitive means to **interactively explore and analyze data**, enabling them to effectively **identify interesting patterns**, **infer correlations and causalities**, and **supports sense-making activities** (Bikakis, [2018](http://arxiv.org/abs/1801.08336)). 

---

class: middle

##### A quick, historical example: have you ever heard of John Snow?
...
While John Snow had to handdraw his map, we have computational resources that we can use to quickly represent our or _his_ data!
...
```{r echo=FALSE, message = FALSE, warning = FALSE, fig.cap = "Recreation of John Snow’s original map of the 1854 cholera outbreak. The size of each black dot is proportional to the number of people who died from cholera at that location. The red dots indicate the location of public water pumps."}
library(tidyverse)
library(mdsr)
library(sf)
library(ggspatial)
...
##### The effectiveness of data visualisation can be relative to the public
...
.center[
.alert[A general rule of thumb: Less is more!]
]

--

<br>
**Disclaimer**: While we will discuss about principles on data visualisation throughout this workshop, our main objective is to show you how to produce figures with `ggplot2`. 

We are limited in time, but we recommend you to interrupt us to discuss about these princples, and to consult other resources: 
...
```{r echo=FALSE, fig.cap = "Using ggplot2::ggplot()"}
# libraries
library(ggplot2)
library(gridExtra)
...
2. It implements the Grammar of Graphics: a standardized, reliable system for building plots;
...
.pull-right[
```{r echo=FALSE, fig.caption = "Using ggplot2::ggplot()"}
# libraries
library(ggplot2)
library(gridExtra)
