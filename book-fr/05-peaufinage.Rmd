...
:::explanation
Bien que les boxplots soient très populaires pour résumer les données, quelqu'un qui regarde votre graphique peut ne pas comprendre intuitivement comment fonctionne ce résumé et ce qu'il peut masquer. Pour bien utiliser les boxplots, il faut en comprendre la composition, comme illustré ci-dessous. Plus d'informations sur les inconvénients des boxplots se trouvent [ici](https://www.data-to-viz.com/caveat/boxplot.html).
...
Pour aider les diagrammes en boîte à parler d'eux-mêmes, nous pouvons ajouter des annotations avec `geom_signif()`. La fonction `geom_signif()` calcule la significativité d'une différence entre les groupes et ajoute l'annotation au graphique en une seule ligne de code.
