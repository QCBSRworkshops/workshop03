...
ggplot(data = penguins, 
       aes(x = bill_length_mm, 
           y = bill_depth_mm)) +
  geom_point() + 
  facet_wrap(~species) # This splits the plot into three windows: one per species
```
...
As a final touch, we can also customise the plot's theme, which includes its background, gridlines, and other visual characteristics of the plot. Let us assume we do not like the default grey background from ggplot2 (we would not be alone in this!). We can use `theme_bw()` to produce a more visually appealing plot. We will discuss themes in Chapter @ref/(theme)!
